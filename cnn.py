...
def cnn(images):
	...
	with tf.variable_scope('local5') as scope:
		...
		weights = variables('weights', [192, 2], stddev=1/192.0)
		bias = variable_on_cpu('bias', [2])
...
def train():
	...
	# data files
	# TensorFlow源码默认下载文件夹
	data_dir = "/home/fuyan/kaggle/dogs_vs_cats/new/"
	# train_dir
	train_dir = '/home/fuyan/kaggle/dogs_vs_cats/new/cat_dog_train/'
	# load data
...
def kaggle_test():
	...
	import csv
	file_test = file('test.csv', 'w')
	writer = csv.writer(file_test)
	writer.writerow(['id', 'label'])
	write_data = []
	for num in range(1, 12501):
		img = cv2.imread('/home/fuyan/kaggle/dogs_vs_cats/resize_test/' + str(num) + '.jpg')
		...
		#---------------------------------------------------------------------------------------------------------
		#-------------------------------------get dog probability---------------------------------------
		y_index = np.exp(np.array(predict[0]))
		y_index = y_index / y_index.sum()
		write_data.append((num, y_index[0]))
		if num % 1000 == 0:
			print num
